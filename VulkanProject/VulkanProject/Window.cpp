...
void VulkanWindow::_CreateTextureImage() 
{
	int texWidth, texHeight, texChannels;
	stbi_uc* pixels = stbi_load("Textures/Me.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
...
void VulkanWindow::_CreateTextureImageView()
{
	_textureImageView = _CreateImageView(_textureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT);
}

//Method to create a new texture sampler
void VulkanWindow::_CreateTextureSampler()
{
...
void VulkanWindow::_CreateDepthResources()
{
	...
	_depthImages.resize(_swapChainImages.size());
	_depthImagesMemory.resize(_swapChainImages.size());
	_depthImagesView.resize(_swapChainImages.size());

	for (size_t i = 0; i < _swapChainImageViews.size(); i++)
	{
		_CreateImage(_swapChainExtent.width, _swapChainExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, _depthImages[i], _depthImagesMemory[i]);
		_depthImagesView[i] = _CreateImageView(_depthImages[i], depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
		_TransitionImageLayout(_depthImages[i], depthFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
	}
}
...
void VulkanWindow::_CleanUpSwapChain()
{
	for (size_t i = 0; i < _swapChainImages.size(); i++)
	{
		vkDestroyImageView(_renderer->GetVulkanDevice(), _depthImagesView[i], nullptr);
		vkDestroyImage(_renderer->GetVulkanDevice(), _depthImages[i], nullptr);
		vkFreeMemory(_renderer->GetVulkanDevice(), _depthImagesMemory[i], nullptr);
	}
